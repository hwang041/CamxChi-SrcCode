...
VOID Feature2MFXRTest::InitializeFeature2Test()
{
    ...
    if (true == bIsCustomFileName)
    {
        ...
        else
        {
            ...
            CF2_LOG_ERROR("An extension is expected in the input file name - %s", inputImage);
        }

        pos = spectraSimConfigImageName.find("_0.raw");

        if (std::string::npos != pos)
        {
            inputImage = spectraSimConfigImageName.erase(pos).append(".raw").c_str();
        }

        else
        {
            result = CDKResultEFailed;
            CF2_LOG_ERROR("The input file name must end with '_0.raw' - %s", inputImage);
        }

        // The metadata and stats dumps have the same name as the input image followed by their respective extensions
        CdkUtils::StrLCpy(inputMetaName, inputImage, size);
    ...
    {
        inputImage = "MFXR_image_4656x3496.raw";
    ...
    m_pTestObj->PatchingMetadata(parsedTintlessBGStats, true);

    // Patching Parsed AWB BG stats to metadata
    m_pTestObj->PatchingMetadata(parsedAWBBGStats, true);

    // Patching Parsed BHIST stats to metadata
    m_pTestObj->PatchingMetadata(parsedBHISTStats, true);
