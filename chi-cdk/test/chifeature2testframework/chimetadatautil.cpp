    ...
    if (m_pInputMetabufferPool == NULL)
    {
        ...
        else
        {
            for (int poolIndex = 0; poolIndex < poolSize; poolIndex++)
            {
                ...
                const CHAR* fileNameString = completeFilePath.str().c_str();
                ChiMetadata* pMetadata = ChiMetadata::Create(fileNameString);
                if (NULL != pMetadata)
                {
...
CDKResult ChiMetadataUtil::PatchingMetabufferPool(const CHAR* pFileName, bool multiFrame)
{
    ...
    for (int poolIndex = 0; poolIndex < m_bufferPoolSize; poolIndex++)
    {
        ...
        const CHAR* fileNameString = completeFilePath.str().c_str();
        FILE*       pFp            = CdkUtils::FOpen(fileNameString, "rb");
        INT         fileLen        = 0;
        CHAR*       pData          = NULL;
        UINT        tagId          = 0;
        ...
        if (pFp)
        {
            ...
            if (0 < fileLen)
            {
                ...
                if (NULL != pData)
                {
                    ...
                    if (0 < bytesRead)
                    {
                        CdkUtils::StrLCpy(lkgFileName, fileNameString, sizeof(lkgFileName));
                        pLKGData = pData;
