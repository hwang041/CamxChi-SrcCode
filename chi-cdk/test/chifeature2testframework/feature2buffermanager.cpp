...
#ifdef _LINUX
    uint32_t width       = pInputStream->pChiStream->width;
    uint32_t height      = pInputStream->pChiStream->height;
    int      format      = pInputStream->pChiStream->format;
    int      dataspace   = pInputStream->pChiStream->dataspace;
    int      usage       = pInputStream->pChiStream->grallocUsage;
    int      planeStride = pInputStream->pChiStream->streamParams.planeStride;
    int      sliceHeight = pInputStream->pChiStream->streamParams.sliceHeight;
    ...
    std::string filename)               //[in] filename to save image as
{
    ...
    else if (HAL_PIXEL_FORMAT_YCbCr_420_888 == format)
    {
        if ((0 != planeStride) || (0 != sliceHeight))
        {
            imageSize = (planeStride * sliceHeight) * 3 / 2;
        }
        else
        {
            imageSize = (output_width * output_height) * 3 / 2;
        }
    }
