...
VOID Feature2OfflineTest::InitializeFeature2Test()
{
    ...
    // Populate a flag which indicates if a custom file name is used for the input image
    ...
    if (true == bIsCustomFileName)
    {
        ...
        const char* inputImageNoExt = std::string(spectraSimConfigImageName.c_str()).erase(pos).c_str();
    ...
    m_pTestObj->PatchingMetadata(&parsedTintlessBGStats[0], false);

    // Patching Parsed AWB BG stats to metadata
    m_pTestObj->PatchingMetadata(&parsedAWBBGStats[0], false);

    // Patching Parsed BHIST stats to metadata
    m_pTestObj->PatchingMetadata(&parsedBHISTStats[0], false);
