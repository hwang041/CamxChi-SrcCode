    ...
    bool multiFrame)                    //[in] indicator for multi frame input images
{
    ...
    CdkUtils::StrLCpy(inputImageName, inputFileName, sizeof(inputImageName));
    char nameBeforeDot[256]  = { 0 };
    char extension[16]       = { 0 };

#ifdef _LINUX
    char *saveptr;
    char* pch = strtok_r(inputImageName, ".", &saveptr);
    if (NULL != pch)
    {
        CdkUtils::StrLCpy(nameBeforeDot, pch, sizeof(nameBeforeDot));
    }
    pch = strtok_r(NULL, ".", &saveptr);
    if (NULL != pch)
    {
        CdkUtils::StrLCpy(extension, pch, sizeof(extension));
    }
#else // _WINDOWS
    char *saveptr;
    char* pch = strtok_s(inputImageName, ".", &saveptr);
    if (NULL != pch)
    {
        CdkUtils::StrLCpy(nameBeforeDot, pch, sizeof(nameBeforeDot));
    }
