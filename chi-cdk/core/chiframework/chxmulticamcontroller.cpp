            ...
            &metadataOpticalZoom, sizeof(OutputMetadataOpticalZoom)))
    {
            ...
            0 == find_camera_metadata_entry(pMetadata, trackerResultROITag, &entry))
        {
            if (entry.count > 0)
            {
                ...
                CHIRECT trackRegions[MaxNumOfT2TROI] = { {0} };
                ...
                for (INT i = 0; i < numOfReceivedT2TROI && i < MaxNumOfT2TROI ; ++i)
                {
                    UINT32      xMin            = entry.data.i32[dataIndex++];
                    UINT32      yMin            = entry.data.i32[dataIndex++];
                    UINT32      xMax            = entry.data.i32[dataIndex++] + xMin;
                    UINT32      yMax            = entry.data.i32[dataIndex++] + yMin;
                    ...
                    else
                    {
                        ...
                        trackRegions[numTrackerROI].width   = xMax - trackRegions[numTrackerROI].left > alignedCropRegion.width
                            ? alignedCropRegion.width : xMax - trackRegions[numTrackerROI].left;
                        trackRegions[numTrackerROI].height  = yMax - trackRegions[numTrackerROI].top > alignedCropRegion.height
