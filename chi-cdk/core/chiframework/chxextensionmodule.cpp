    ...
    VOID**                          pPrivate)
{
    ...
    *pIsOverrideEnabled   = FALSE;
    ...
    if (CDKResultSuccess != result)
    {
        // failure case. reset resource count
        ResetResourceCost(m_logicalCameraInfo[logicalCameraId].cameraId);
    }

    CHX_LOG_INFO(" m_totalResourceBudget=%d,  total resource used so far = %d",
            m_totalResourceBudget, m_IFEResourceCost[logicalCameraId]);
    ...
    VOID*                       pPriv)
{
    ...
    if (CDKInvalidId != logicalCameraId)
    {
        ...
        if ((result == CamxResultECancelledRequest) &&
            (TRUE == static_cast<BOOL>(ChxUtils::AtomicLoadU32(&m_aFlushInProgress[logicalCameraId]))))
        {
    ...
    UINT32 logicalCameraId)
{
    ...
    {
        m_pTriggerRecoveryLock[logicalCameraId]->Lock();
        if (FALSE == m_IsRecoverySignaled[logicalCameraId])
        {
    ...
    UINT32 cameraID)
{
    ...
    m_pHALOps->process_capture_result(m_logicalCameraInfo[cameraID].m_pCamera3Device, pResult);
...
VOID ExtensionModule::ReturnFrameworkMessage(
    ...
    UINT32 cameraID)
{
    m_pHALOps->notify_result(m_logicalCameraInfo[cameraID].m_pCamera3Device, pMessage);
}
    ...
    StreamMap*                      pStreamMapData)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        for (UINT stream = 0; stream < pStreamConfig->num_streams; stream++)
        {
            ...
            if ((pStream->format == p_stream->format) &&
            ...
            if (0 == find_camera_metadata_entry(metadata, tagLocation, &entry))
            {
                ...
                for (UINT32 i = 0; i < entry.count;)
                {
                        ...
                        (pStream->format == pStreamMapData->format))
                    {
                        ...
                        if ((pStream->width * pStream->height) < SnapshotYUVOutputCap)
                        {
