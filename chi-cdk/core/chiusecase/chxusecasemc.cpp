        ...
        if (TRUE == m_requestMapInfo[internalFrameNumIndex].isPreviewReturnNeeded)
        {
            ...
            for (UINT32 j = 0; j < pResult->numOutputBuffers; j++)
            {
                ...
                if (((m_pTargetPreviewStream == reinterpret_cast<CHISTREAM*>(pResultBuffer->stream))     ||
                     (m_pTargetYUVPreviewStream == reinterpret_cast<CHISTREAM*>(pResultBuffer->stream))) &&
    ...
    VOID*                       pPrivateCallbackData)
{
    ...
    {
        CHX_LOG("Notify frameNum: %d, pSessionPrivateData or pMessageDescriptor->pPrivData is NULL", internalFrameNum);
    }
    ...
    VOID*             pPrivateCallbackData)
{
    ...
    CHX_LOG_INFO("Result: Session = %d Pipeline = %d FrameNum: %d buffer - type %d handle %p, status - %d "
            "sync_count = %d numbuf = %d partial = %d input = %p",
            pSessionPrivateData->sessionId,
            pipelineIndex,
            pResult->frameworkFrameNum,
        ...
        BOOL                            doCrossPipelineBufferNegotiations)
{
    ...
    CHIBUFFEROPTIONS* pDSOptions        = NULL; ///< @todo remove assumption these are shared
    ...
    for (UINT pipelineNumber = 0; pipelineNumber < numPipelines; pipelineNumber++)
    {
        ...
        if (CDKResultSuccess == result && TRUE == bCreatePipeline)
        {
            ...
            if (NULL != m_pPipeline)
            {
                ...
                {
                    auto SetRealtimeBufferNegotiations = [&BufferDesc](const CHIBUFFEROPTIONS* pOptions, UINT streamIndex)
                    {
                        if (NULL != pOptions && INVALID_INDEX != streamIndex)
                        {
                            BufferDesc[streamIndex].hasValidBufferNegotiationOptions  = TRUE;
                            BufferDesc[streamIndex].pBufferNegotiationsOptions        = pOptions;
                            CHX_LOG_INFO("Assigning buffer requirements %p to idx: %u", pOptions, streamIndex);
                        }
                        else
                        {
                            CHX_LOG_WARN("Invalid State: buffer requirements %p idx: %u", pOptions, streamIndex);
                        }
                    };
                    SetRealtimeBufferNegotiations(pFullOptions, rtIndexFull);
                    SetRealtimeBufferNegotiations(pDSOptions, rtIndexDS4);
                    SetRealtimeBufferNegotiations(pDSOptions, rtIndexDS16);
                }
                ...
                if (UsecaseSAT == m_usecaseMode)
                {
                    // We cannot defer the initialization of the realtime pipeline while doing cross pipeline buffer negotations
                    // because the RT Pipelines' forward walk must occur before the creation of this Usecase's internal buffers
                    BOOL deferInit = ((FALSE == doCrossPipelineBufferNegotiations) && (REALTIME_SESSION == sessionIndex));
                    m_pPipeline->SetDeferFinalizeFlag(deferInit);
                }
                    ...
                    if (OFFLINE_YUV_SESSION == sessionIndex)
                    {
                        ...
                        if (TRUE == doCrossPipelineBufferNegotiations)
                        {
                            ...
                            for (UINT buffIdx = 0; buffIdx < sourceIndex; buffIdx++)
                            {
                                ...
                                else if (TRUE == IsSameDesc(targetBufferInfo.mapDS4[0]))
                                {
                                    pDSOptions = &pDesc->bufferOptions;
                                }
                            }
    ...
    for (UINT32 i = 0; i < m_numOfPhysicalDevices; i++)
    {
        ...
        if (FALSE == m_isSnapshotYUVNeeded)
        {
            ...
            if (NULL != m_pTargetSnapshotStream)
            {
                ...
                FLOAT rdiRatio = RDIMaxHeight / static_cast<FLOAT>(RDIMaxWidth);
                    ...
                    for (UINT32 i = 0; i < requestForFeature2.num_output_buffers; i++)
                    {
                        ...
                        for (UINT32 i = 0; i < m_numOfPhysicalDevices; i++)
                        {
                            if (pRequestOutputStream == m_multiCamResource[i].m_pTargetYUVStream)
                            {
                                ...
                                CHX_LOG_INFO("actve camera id:%s", pRequestOutputStream->physicalCameraId);
                            }
                        }
                    }
    ...
    const CHICAPTURERESULT* pResult)
{
    ...
    CHX_LOG("pipeline idx:%d internalFrameNumIndex:%u", pipelineIndex, internalFrameNumIndex);
    for (UINT32 i = 0; i < pResult->numOutputBuffers; i++)
    {
        ...
        pSessionInfo->m_bufferQueue[internalFrameNumIndex].m_resultMask))
    {
        ...
        if (CDKResultSuccess == result)
        {
            ...
            CHX_LOG("Metadata internalFrameNumIndex %d (%p %p %p %p)", internalFrameNumIndex,
                   pSessionInfo->m_bufferQueue[internalFrameNumIndex].pMetadata[0],
   ...
   ChiMetadata*               pFeatureInputMetadata)
{
    ...
    ActivateDeactivateRealtimePipeline(&m_MCCResult, internalFrameNum, pRequest);
    ...
    for (UINT32 pipelineIndex = 0; pipelineIndex < m_numOfPhysicalDevices; pipelineIndex++)
    {
        ...
        for (UINT32 stream = 0; stream < pRequest->num_output_buffers; stream++)
        {
            ...
            else if (pStream == m_pTargetSnapshotStream)
            {
                ...
                {
                    CHX_LOG("Snapshot acquire = %d release = %d SnapshotMask=%d pipelineid = %d", m_appSnapshotBuffer[applicationFrameNumIndex].acquire_fence,
                            m_appSnapshotBuffer[applicationFrameNumIndex].release_fence, m_requestInfo[internalFrameNumIndex].m_snapshotMask,
                            pipelineIndex);
    ...
    if (0 < numOfRequest)
    {
        CHX_LOG_INFO("NumReq:%d Active:0x%x, Preview:0x%x, Snapshot:0x%x,"
                     "Raw:0x%x, PreviewYUV:0x%x, FrameNum:%d",
                     numOfRequest,
                     m_requestInfo[internalFrameNumIndex].m_activeMask,
                     m_requestInfo[internalFrameNumIndex].m_previewMask,
                     m_requestInfo[internalFrameNumIndex].m_snapshotMask,
        ...
        UINT32 bufferIndex)
{
    ...
    {
        CHX_LOG_ERROR("[MultiCamera RDI] Request : frameNum= %" PRIu64 " acquireFence: (valid=%d type=%d nativeFence=%d chiFence=%p) "
