        ...
        for (UINT32 i = 0; i < pExtSrcStreamData->numInputs; i++)
        {
            ...
            if (CDKResultSuccess == result)
            {
                ...
                result = pRequestInternalInputTBM->ImportExternalTargetBuffer(
                    internalFrameNumber + i, reinterpret_cast<UINT64>(pInternalStream), &inputBuffer);

                // Setup Buffer TBH
                hOutputTBH = pRequestInternalInputTBM->SetupTargetBuffer(internalFrameNumber + i);

                // Update status to ready
                pRequestInternalInputTBM->UpdateTarget(internalFrameNumber + i,
                                                       reinterpret_cast<UINT64>(pInternalStream),
                                                       inputBuffer.bufferInfo.phBuffer,
                                                       ChiTargetStatus::Ready,
                                                       &hOutputTBH);

                CHX_LOG_INFO("Got rdi buffer %p TBH %p, pipelineindex:%d",
                ...
                if (NULL != pSrcMetadata)
                {
                    ...
                    result = pRequestInputMetadataTBM->ImportExternalTargetBuffer(
                        internalFrameNumber + i, m_metaClientId, pSrcMetadata);

                    // Setup Metadata TBH
                    hMetadataTBH = pRequestInputMetadataTBM->SetupTargetBuffer(internalFrameNumber + i);

                    // Update status to ready
                    pRequestInputMetadataTBM->UpdateTarget(internalFrameNumber + i,
                                                           m_metaClientId,
                                                           pSrcMetadata,
                                                           ChiTargetStatus::Ready,
                                                           &hMetadataTBH);

                    CHX_LOG_INFO("Got rdi buffer %p meta %p,hmeta:%p pipelineindex:%d",
    ...
    if (NULL != pUsecaseRequestObject)
    {
        ...
        for (UINT32 j = 0; j < pResult->numOutputBuffers; j++)
        {
                ...
                (TRUE == m_pUsecaseBase->IsMultiCameraUsecase()))
            {
                ...
                CHX_LOG_INFO("processresult yuv buffer %p PrivData %p StreamIndex = %d",
    ...
    const camera_metadata_t* pMetadata)
{
    ...
    return MaxFrameCntNeededForFeature2;
}
