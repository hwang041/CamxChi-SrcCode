...
VOID CHITargetBufferManager::Destroy()
{
    ...
    for (UINT i = 0; i < ListSize; i++)
    {
        if (NULL != pLists[i])
        {
            ...
            if (NULL != pLists[i]->Head())
            {
                firstSeqId = reinterpret_cast<ChiTargetBufferInfo*>(pLists[i]->Head()->pData)->seqId;
            }

            if (NULL != pLists[i]->Tail())
            {
                lastSeqId = reinterpret_cast<ChiTargetBufferInfo*>(pLists[i]->Tail()->pData)->seqId;
            }

            if ((INVALIDSEQUENCEID != firstSeqId) && (INVALIDSEQUENCEID != lastSeqId))
            {
                for (UINT32 seqId = firstSeqId; seqId <= lastSeqId; seqId++)
                {
            ...
            if (NULL != pSrcTarget)
            {
                if (m_targetType[targetIdx] == ChiTargetType::Metadata)
                {
                    ...
                    if (pSrcMetadata == pTargetBufferInfo->pMetadata[targetIdx])
                    {
                        ...
                        CHX_LOG("[%s] seqId=%d targetIdx=%d pMetadata=%p status updated to %d",
                            m_targetBufferName, seqId, targetIdx, pSrcMetadata, status);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (hBuffer == pTargetBufferInfo->chiStreamBuffer[targetIdx].bufferInfo.phBuffer)
                    {
                        ...
                        CHX_LOG("[%s] seqId=%d targetIdx=%d phBuffer=%p status updated to %d",
                                m_targetBufferName, seqId, targetIdx, hBuffer, status);
                    }
                    else
                    {
    ...
    if (CDKResultSuccess == result)
    {
        ...
        if (TRUE == waitFlag && NULL == pCallbackData)
        {
                   ...
                   (NULL != SearchList(seqId, SearchOption::SearchProducerList)))
            {
                ...
                result = m_pCondition->TimedWait(m_pLock->GetNativeHandle(), WAIT_BUFFER_TIMEOUT);
    ...
    if (NULL != hTargetBufferInfo)
    {
        ...
        if (0 == pTargetBufferInfo->refCount)
        {
            ...
            CHX_LOG_INFO("[%s]_TBM Info: MinCnt:%d MaxCnt:%d ProdQ:%d ConsumerQ:%d"
                ...
                reinterpret_cast<ChiFence*>(pTargetBufferInfo->chiStreamBuffer[0].releaseFence.hChiFence)->hFence : 0,
    ...
    UINT32 seqId)
{
    ...
    if (NULL != pNode)
    {
        ...
        {
            CHX_LOG("[%s] target buffer of seqId=%d is ready, move to consumer list", m_targetBufferName, seqId);
            ...
            {
                CHX_LOG_INFO("[%s] target buffer of seqId=%d is ready callback", m_targetBufferName, seqId);
        ...
        {
            CHX_LOG("[%s] target buffer of seqId=%d is ready but refCt=0, remove it", m_targetBufferName, seqId);
        ...
        {
            CHX_LOG("[%s] target buffer of seqId=%d is in error status, remove it", m_targetBufferName, seqId);
    ...
    UINT32 seqId)
{
    ...
    if (NULL != pNode)
    {
        ...
        if (0 < pTargetBufferInfo->refCount)
        {
            CHX_LOG_WARN("[%s] Target buffer seqId=%d is removed with refCount=%d",
        ...
        for (UINT i = 0; i < m_numOfTargets; i++)
        {
            if (m_targetType[i] == ChiTargetType::Metadata)
            {
                if (NULL != pTargetBufferInfo->pMetadata[i])
                {
                    CHX_LOG("[%s] Release seqId %d metadata=%p, isImported:%d",
                        m_targetBufferName, pTargetBufferInfo->seqId, pTargetBufferInfo->pMetadata[i],
                        pTargetBufferInfo->isImported);
            ...
            else if (m_targetType[i] == ChiTargetType::InternalBuffer)
            {
                if (NULL != pTargetBufferInfo->chiStreamBuffer[i].bufferInfo.phBuffer)
                {
                    CHX_LOG("[%s] Release seqId %d phBuffer=%p, isImported:%d",
                        m_targetBufferName, pTargetBufferInfo->seqId,
                        pTargetBufferInfo->chiStreamBuffer[i].bufferInfo.phBuffer,
                        pTargetBufferInfo->isImported);
                    if (FALSE == pTargetBufferInfo->isImported)
