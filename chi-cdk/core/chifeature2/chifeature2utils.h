    ...
    CHAR*            pName)
{
    ...
    INT stride = 0;
    // Only support raw10 and raw opaque and raw16 for now
    if (ChiStreamFormatRaw10 == pStreamBuffer->pStream->format)
    {
        stride = pStreamBuffer->pStream->width * 10 / 8;
        stride = ChxUtils::AlignGeneric32(stride, 16);
    }
    else if (ChiStreamFormatRawOpaque == pStreamBuffer->pStream->format)
    {
        stride = pStreamBuffer->pStream->width * 10 / 8;
    }
    else if (ChiStreamFormatRaw16 == pStreamBuffer->pStream->format)
    {
        stride = pStreamBuffer->pStream->width * 2;
    }
    else
    {
        CHX_LOG_ERROR("Raw format not supported: 0X%x", pStreamBuffer->pStream->format);
    }
