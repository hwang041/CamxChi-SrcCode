    ...
    ) const
{
    ...
    if ((CDKResultSuccess == result) && (NULL != pFeatureSettings) && (NULL!= pFeatureHint))
    {
        if (TRUE == pFeatureHint->captureMode.u.LowLight)
        {
            controlZSLValue= ANDROID_CONTROL_ENABLE_ZSL_FALSE;
        }
    ...
    ) const
{
    ...
    if ((CDKResultSuccess == result) && (NULL != pMetadata))
    {
        ...
        // DumpStats(pMetadata, index, pBaseName);
        if (0 == index)
        {
            DumpFileName* pData         = NULL;
            DumpFileName  fileNameArray = { { { 0 } } };
            ChxUtils::GetVendorTagValue(pMetadata, VendorTag::DebugDumpConfig, (VOID**)&pData);
            if (NULL != pData)
            {
                ChxUtils::Memcpy(&fileNameArray, pData, sizeof(DumpFileName));
            }
            UINT32 physicalCamId = m_pCameraInfo->ppDeviceInfo[m_pInstanceProps->cameraId]->cameraId;
            ChxUtils::Memcpy(fileNameArray.dumpFileName[physicalCamId], pBaseName, 128);
            ChxUtils::SetVendorTagValue(pMetadata, VendorTag::DebugDumpConfig, sizeof(DumpFileName), &fileNameArray);
        }
    }
