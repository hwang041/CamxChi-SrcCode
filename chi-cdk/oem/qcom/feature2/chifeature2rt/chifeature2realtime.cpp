    ...
    ChiFeature2CreateInputInfo* pCreateInputInfo)
{
    ...
    else
    {
        ...
        {
            UINT maxHalRequests = ExtensionModule::GetInstance()->GetMaxHalRequests() - 1;
    ...
    ) const
{
    ...
    for (UINT8 portIndex = 0; portIndex < pAllOutputPorts->numPorts; ++portIndex)
    {
        ...
        else
        {
            ...
            // Avoid continuous RDI requests in video usecases,
            // unless we are in BPS camera usecase as it always needs RAW
            // buffers for its Realtime stage.
            if (RealtimeOutputPortRaw == portId)
            {
                ...
                else if ((TRUE == m_isVideoStreamEnabled) &&
                         (IsPortEnabledInFinalOutput(pRequestObject,
                                                     RealtimeOutputPortVideo,
                                                     pRequestObject->GetCurRequestId())))
                {
                    if (((NULL != pInstanceprops) && (TRUE == pInstanceprops->instanceFlags.isBPSCamera)) ||
                        (IsPortEnabledInFinalOutput(pRequestObject,
                                                    RealtimeOutputPortRaw,
                                                    pRequestObject->GetCurRequestId())))
                    {
            ...
            // Always add FD port for BPS camera
            else if ((RealtimeOutputPortFd == portId) &&
                     (NULL != pInstanceprops) &&
                     (TRUE == pInstanceprops->instanceFlags.isBPSCamera))
            {
                selectedList.push_back(RealtimeOutputPortFd);
            }
            // Always add RT display output port for BPS camera
            else if ((RealtimeOutputPortDisplay == portId) &&
                     (NULL != pInstanceprops) &&
                     (TRUE == pInstanceprops->instanceFlags.isBPSCamera))
            {
                selectedList.push_back(RealtimeOutputPortDisplay);
            }
            else if (RealtimeOutputPortFd == portId)
            {
                if (((TRUE == (IsPortEnabledInFinalOutput(pRequestObject,
    ...
    ) const
{
    ...
    else if (TRUE == pCreateInputInfo->pInstanceProps->instanceFlags.isBPSCamera)
    {
    ...
    else
    {
        ...
        {
            CHX_LOG_INFO("featureName:%s, pipelineName:%s, instanceFlags {nzsl:%d, swremosaic:%d, hwremosaic:%d}",
