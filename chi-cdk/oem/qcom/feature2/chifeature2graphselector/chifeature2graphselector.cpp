...
extern std::map<std::set<UINT>, UINT> cameraIdMap;
...
VOID ChiFeature2GraphSelector::PopulateAllTables()
{
    ...
    keys =
    {
        ...
        SINGLE_CAMERA,{ ControlCaptureIntentStillCapture,
                        ...
                        ControlCaptureIntentManual },                        { ControlSceneModeHDR },                 { NoiseReductionModeHighQuality },             { 0 },            0
    };
    ...
    std::vector<FGDKeysForTable>* FeatureGraphDesc)
{
    ...
    for (auto &it : *FeatureGraphDesc)
    {
        ...
        for (auto &cameraMapIt : cameraIdMap)
        {
                    ...
                    if (it.noiseReductionMode.find(pKeysForRequest->noiseReductionMode) != it.noiseReductionMode.end())
                    {
                        if (it.sceneMode.find(pKeysForRequest->sceneMode) != it.sceneMode.end())
                        {
                            ...
                            if ((FALSE == phyCamDesc.bMultiCamera) &&
                                (FALSE == phyCamDesc.featureFlags.isNZSLSnapshot) &&
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
VOID ChiFeature2GraphSelector::GetPruneRulesAndIdentifier(
        const FGDKeysForCaptureRequest*               pKeysForRequest,
        ...
        const FGDKeysForCaptureRequest* pKeysForRequest)
{
    ...
    if (CDKResultSuccess == result)
    {
        ...
        CHAR pruneIDString[MaxStringLength256];

        GetPruneRulesAndIdentifier(pKeysForRequest, pruneRules, pruneIDString);

        // Search pruned graph descriptor in cache first
        if (0 != CdkUtils::StrLen(pruneIDString))
        {
...
VOID ChiFeature2GraphSelector::BuildCameraIdSet()
{
    ...
    cameraIdMap.clear();
    cameraIdMap.insert({ { cameraIdSetSingle },  SINGLE_CAMERA });
    cameraIdMap.insert({ { cameraIdSetMulti },   MULTI_CAMERA });
    cameraIdMap.insert({ { cameraIdSetBokeh },   BOKEH_CAMERA });
    cameraIdMap.insert({ { cameraIdSetFusion },  FUSION_CAMERA });
