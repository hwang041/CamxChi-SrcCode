    ...
    struct v4l2_event* pEvent)
{
    ...
    if ((0 == pMessage->u.frame_msg.request_id) &&
        (pEvent->id != V4L_EVENT_CAM_REQ_MGR_SOF)) // Ignore event if request_id 0 for multi camera single zone
    {
    ...
    UINT32        opMode)
{
    ...
    struct cam_req_mgr_link_control  linkControl;
    CamxResult                       result     = CamxResultEFailed;
            ...
            if (NULL != pHWDevice->deviceOp.StreamOn)
            {
                ...
                if (CamxResultSuccess != result)
                {
                    ...
                    CAMX_LOG_ERROR(CamxLogGroupCSL,
                        "Stream On Failed DevH: 0x%x, hAc: 0x%x type: %d, idx: %d name:%s",
                        phDevices[i_temp],
                        pAcquireddevice->hAcquired,
                        pAcquireddevice->deviceType,
                        pAcquireddevice->cslDeviceIndex,
    ...
    CSLDeactivateMode                   mode)
{
    ...
    while (NULL != pNode)
    {
        ...
        if ((NULL != pHWDevice->deviceOp.StreamOff) && (TRUE == turnOffDevice))
        {
            ...
            if (CamxResultSuccess != result)
            {
                CAMX_LOG_ERROR(CamxLogGroupCSL, "Stream off failed: 0x%x, hAc: 0x%x type: %d, idx: %d, name = %s, result = %d",
                    phDevices[i_temp],
                    pAcquireddevice->hAcquired,
                    pAcquireddevice->deviceType,
                    pAcquireddevice->cslDeviceIndex,
    ...
    CAMX_LOG_INFO(CamxLogGroupCSL, "Stream off mode %d Done = %d", mode, result);
    return result;
}
