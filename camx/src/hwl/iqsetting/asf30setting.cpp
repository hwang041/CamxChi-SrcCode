    ...
    ASF30UnpackedField*                 pUnpackedField)
{
    ...
    UINT32 lutVal1;
    UINT32 lutVal2;
    INT32  maxUint8 = static_cast<INT32>(MAX_UINT8);
    INT    index;
    FLOAT  ratio;
        ...
        for (j = 0; j < interpolationPoints; j++)
        {
            ...
            if (i < LUT_GAINWEIGHT_SIZE - 1)
            {
                ...
                pUnpackedField->layer1GainWeightLut[index] =
                    static_cast<UINT8>(IQSettingUtils::ClampINT32(MAX_UINT8 - lutVal1, MIN_UINT8, MAX_UINT8));

                pUnpackedField->layer2GainWeightLut[index] =
                    static_cast<UINT8>(IQSettingUtils::ClampINT32(MAX_UINT8 - lutVal2, MIN_UINT8, MAX_UINT8));
            }
            else
            {
    ...
    ASF30UnpackedField*                 pUnpackedField)
{
    ...
    if (pUnpackedField->maxY > pUnpackedField->minY)
    {
        pUnpackedField->paraSmin = IQSettingUtils::ClampUINT32(IQSettingUtils::FloatToQNumber(
            (minSlY - minShY) / (pUnpackedField->maxY - pUnpackedField->minY), SKIN_SATURATIN_Q_FACTOR),
            MIN_UINT8, MAX_UINT8);
        pUnpackedField->paraSmax = IQSettingUtils::ClampUINT32(IQSettingUtils::FloatToQNumber(
            (maxSlY - maxShY) / (pUnpackedField->maxY - pUnpackedField->minY), SKIN_SATURATIN_Q_FACTOR),
            MIN_UINT8, MAX_UINT8);
    }
    else
    {
