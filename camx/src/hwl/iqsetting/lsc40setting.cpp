    ...
    VOID*                                                 pOutput)
{
        ...
        (NULL != pOutput))
    {
                ...
                for (int rowCount = 0; rowCount <= number_row; rowCount++)
                {
                    for (int colCount = 0; colCount <= number_column; colCount++)
                    {
                        pUnpackedField->grids_gain[1][rowCount][colCount] = IQSettingUtils::ClampUINT16(
                            ...
                            LSC40_GRID_GAIN_MIN,
                            LSC40_GRID_GAIN_MAX);
                        pUnpackedField->grids_gain[0][rowCount][colCount] = pUnpackedField->grids_gain[1][rowCount][colCount];
                        ...
                        pUnpackedField->grids_mean[1][rowCount][colCount] = IQSettingUtils::ClampUINT16(
                            ...
                            LSC40_GRID_MEAN_MIN,
                            LSC40_GRID_MEAN_MAX);
                        pUnpackedField->grids_mean[0][rowCount][colCount] = pUnpackedField->grids_mean[1][rowCount][colCount];
                    }
                }
            }
            else
        ...
        {
            pUnpackedField->ALSC_enable = 0;
            CAMX_LOG_ERROR(CamxLogGroupISP, "AWB BG Stat is not available");
        }
    }
    else
    {
