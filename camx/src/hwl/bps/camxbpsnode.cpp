    ...
    ISPInputData* pInputData)
{
    ...
    if (TRUE == IsRealTime())
    {
        ...
        static VOID* pProperties3AData[PropertiesSize] = { 0 };
    ...
    if (TRUE == updateStats)
    {
        ...
        if ((GetMaximumPipelineDelay() < requestIdOffsetFromLastFlush) || (FALSE == IsRealTime()))
        {
            ...
            if (NULL != ppPropertyData3A[5])
            {
                ...
                {
                    CAMX_LOG_VERBOSE(CamxLogGroupBPS, "TLBG Config %d %d | %d TLBG stats %x %x %x ",
        ...
        ((1 == sequenceNumber) || ((0 == sequenceNumber) && (sensorConnected == FALSE) && (FALSE == hasDependency))))
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (NULL != pImageFormat)
            {
                ...
                {
                    moduleInput.tuningModeChanged = TRUE;
                    UpdateICAChromatixGridData(cameraId);
                }
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if (NULL != pImageFormat)
            {
                ...
                CAMX_LOG_INFO(CamxLogGroupBPS, "node %s Input : firmwarePortId %d format %d, width %d, height %d",
                              NodeIdentifierString(),
                ...
                for (outputPortIndex = 0; outputPortIndex < numOutputPort; outputPortIndex++)
                {
                    ...
                    CAMX_LOG_INFO(CamxLogGroupBPS, "node %s O/P: firmwarePortId %d format %d,"
    ...
    CmdBuffer*    pCmdBufferBlob)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        for (count = 0; count < m_numIQMOdules; count++)
        {
            ...
            if (CamxResultSuccess != result)
            {
                CAMX_LOG_ERROR(CamxLogGroupBPS, "Failed to Run IQ Config, count %d", count);
            }
            if (TRUE == adrcEnabled &&
    ...
    const ISPInputData* pInputData)
{
    ...
    else
    {
            ...
            (BPSProfileId::BPSProfileIdIdealRawInput == m_instanceProperty.profileId)))
        {
            ...
            ppPostLSCData[index]     = &pInputData->pCalculatedData->toneMapData.tonemapMode;
            index++;
    ...
    if (CamxResultSuccess == result)
    {
        if ((TRUE == m_publishLDCGridData) &&
            ((0 != m_vendorTagArray[ICAInGridOut2InTransformIdx]) ||
            (0 != m_vendorTagArray[ICAInGridIn2OutTransformIdx])))
        {
    ...
    const SensorMode* pSensorMode)
{
    ...
    if ((TRUE == m_ICAGridOut2InEnabled) && (TRUE == m_ICAGridIn2OutEnabled))
    {
            ...
            (0 != m_ldFullOutSize.heightLines))
        {
            ...
            {
                NcLibWindowRegion windowRegion;
                windowRegion.fullWidth    = m_ldFullOutSize.widthPixels;
                windowRegion.fullHeight   = m_ldFullOutSize.heightLines;
                windowRegion.windowLeft   = pSensorMode->cropInfo.firstPixel;
                windowRegion.windowTop    = pSensorMode->cropInfo.firstLine;
                windowRegion.windowWidth  = m_sensorSize.widthPixels;
                windowRegion.windowHeight = m_sensorSize.heightLines;
    ...
    NodeMetadataList* pPublistTagList)
{
    ...
    if (NumBPSMetadataOutputTags + BPSMaxOutputICAVendorTags < MaxTagsPublished)
    {
        ...
        if (TRUE == m_publishLDCGridData)
        {
            ...
            {
                if (((FALSE == m_ICAGridOut2InEnabled) && (ICAInGridOut2InTransformIdx == tagIndex)) ||
                    ((FALSE == m_ICAGridIn2OutEnabled) && (ICAInGridIn2OutTransformIdx == tagIndex)))
                {
                    continue;
                }
    ...
    UINT32 cameraId)
{
        ...
        (TRUE == pTuningManager->IsValidChromatix()))
    {
        switch (m_capability.ICAVersion)
        {
            case ICAVersion30:
                pICAChromatix =
                    static_cast<VOID *>(pTuningManager->GetChromatix()->GetModule_ica30_ipe_module1(selectors, numSelectors));
                break;
            case ICAVersion20:
                pICAChromatix =
                    static_cast<VOID *>(pTuningManager->GetChromatix()->GetModule_ica20_ipe_module1(selectors, numSelectors));
                break;
            default:
                break;
        }
