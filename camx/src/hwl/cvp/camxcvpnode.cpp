...
VOID CVPNode::Cleanup()
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupCVP, "Cleanup invoked");
    ...
    UINT          parentNodeId)
{
    ...
    if (TRUE == bIsFDPostingResultsEnabled)
    {
        ...
        else
        {
            ...
            {
                CAMX_LOG_ERROR(CamxLogGroupCVP, "Face ROI is not published");
            }
        }
    }
    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupCVP, "Exit Req %llu, seq %d", requestId, pNodeRequestData->processSequenceId);
...
CamxResult CVPNode::PostPipelineCreate()
{
    ...
    if ((NULL == m_ppGridBuffers) ||
        (NULL == m_ppSourceContextBuffers) ||
        (NULL == m_phSynxSrcImage) ||
    ...
    if ((CamxResultSuccess == result) && (FALSE == m_disableCVPDriver))
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupCVP, "RModel %u inlierTrack %u tarVar %f ncc diff %f",
    ...
    UINT8  forceIdentityTransform)
{
    ...
    perspectiveTransform.byPassAlignmentMatrixAdjustement = (1 == forceIdentityTransform) ? TRUE : FALSE;
    ...
    else
    {
        ...
        if (CamxResultSuccess == result)
        {
            CAMX_LOG_VERBOSE(CamxLogGroupCVP, "CVP:%d posted ICA : Confidence %d res %dX%d for req %llu",
                            InstanceID(), confidence, height, width, requestId);
            CAMX_LOG_VERBOSE(CamxLogGroupCVP, "[%lf %lf %lf] [%lf %lf %lf] [%lf %lf %lf] persp enable %d",
        ...
        {
            CAMX_LOG_ERROR(CamxLogGroupCVP, "Failed to publish ICA transform");
        }
    }
    ...
    UINT   portId)
{
    ...
    if (TRUE == m_disableCVPDriver)
    {
        ...
        {
            result =
                PostICATransform(&MCTFtransform, MaxTransformConfidence, m_fullInputWidth, m_fullInputHeight, requestId, 1);
        }
    }
    else
    {
        ...
        UINT8  forceIdentityTransform   = 0;
            ...
            if (pOutputPort->portId == CVPOutputPortData)
            {
                if (NULL != pBuffer)
                {
                    ...
                    if (NULL != pDmeOutput)
                    {
                        ...
                        CAMX_LOG_INFO(CamxLogGroupCVP, "%s: scaleFactorW %f,scaleFactorH %f,coarseCenterW %f,coarseCenterH %f",
                                      NodeIdentifierString(), m_scaleFactorW, m_scaleFactorH, m_coarseCenterW, m_coarseCenterH);


                        if (CamxResultSuccess == result)
                        {
                            result = ConfigureCVPConfidenceParameter(&transformConfidence, &forceIdentityTransform);
                        }
                        ...
                        {
                            CAMX_LOG_ERROR(CamxLogGroupCVP, "Req[%llu] Failed in Configure CVP Confidence Param, result=%d",
                                           requestId, result);
                        }
                        ...
                        if (TRUE == IsCVPInPrefilter())
                        {
                            ...
                            //  The requestIdIndex will be parsed in blend stage and be used to prevent the redundant memcpy.
...
VOID CVPNode::PrintMatrix(
    ...
    UINT64                  requestId)
{
    CAMX_LOG_VERBOSE(CamxLogGroupCVP, "Req %llu confidence %d [%lf %lf %lf] [%lf %lf %lf] [%lf %lf %lf]",
    ...
    UINT32  requestIdIndex)
{
    ...
    if (( 0 != m_phSynxDmeOutput[requestIdIndex]) &&
        (SYNX_STATUS_SIGNALED_SUCCESS == synx_get_status(m_phSynxDmeOutput[requestIdIndex])))
    {
    ...
    if ((TRUE == m_bICAEnabled) &&
        ...
        (SYNX_STATUS_SIGNALED_SUCCESS == synx_get_status(m_phSynxIcaOutput[requestIdIndex])))
    {
    ...
    if (( 0 != m_phSynxSrcImage[requestIdIndex]) &&
        (SYNX_STATUS_SIGNALED_SUCCESS == synx_get_status(m_phSynxSrcImage[requestIdIndex])))
    {
...
{
    CamxResult                      result = CamxResultSuccess;
    TuningDataManager*              pTuningManager = GetTuningDataManagerWithCameraId(GetPipeline()->GetCameraId());
