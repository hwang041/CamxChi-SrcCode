    ...
    ISPInputData* pModuleInput)
{
    ...
    switch(m_instanceProperty.processingType)
    {
        ...
        case IPEProcessingType::IPEMFNRPostfilter:
            if (NULL != pData[4])
            {
                ...
                CAMX_LOG_INFO(CamxLogGroupPProc, "Total number of MFNR Frames = %d",
        ...
        case IPEProcessingType::IPEMFSRPostfilter:
            if (NULL != pData[5])
            {
                ...
                CAMX_LOG_INFO(CamxLogGroupPProc, "Total number of MFSR Frames = %d",
                    pModuleInput->pipelineIPEData.numOfFrames);
            }
            else
            {
    ...
    {
        CAMX_LOG_INFO(CamxLogGroupPProc, "Cannot obtain blue tonemap curve");
    }
    ...
    {
        CAMX_LOG_INFO(CamxLogGroupPProc, "Cannot obtain green tonemap curve");
    }
    ...
    {
        CAMX_LOG_INFO(CamxLogGroupPProc, "Cannot obtain red tonemap curve");
    }
    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    if (1 == sequenceNumber)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            {
                if (FALSE == m_capability.isPostfilterWithBlend)
                {
                    moduleInput.mfFrameNum = (requestId - 1) % (moduleInput.pipelineIPEData.numOfFrames - 1) + 1;
                }
                else
                {
                    moduleInput.mfFrameNum = (requestId - 1) % (moduleInput.pipelineIPEData.numOfFrames - 2) + 1;
                }
                CAMX_LOG_VERBOSE(CamxLogGroupPProc, "[%s]: request=%d, total frame=%d, FrameNum = %d",
        ...
        if (CamxResultSuccess == result)
        {
            ...
            else
            {
                ...
                {
                    result = CamxResultEFailed;
                    CAMX_LOG_ERROR(CamxLogGroupPProc, "Sensor Mode Data is NULL.");
                }
            }
        }
    ...
    const CHAR*        pPortType)
{
    ...
    if (TRUE == IsValidConfigIOData(pConfigIOData->images[firmwarePortId].info.format, firmwarePortId))
    {
    ...
    IPETuningMetadata** ppTuningMetadata)
{
    ...
    else
    {
        ...
        // Don't dump debug data if in Scale mode
        if ((CamxResultSuccess == result) && (IPEProfileIdScale != m_instanceProperty.profileId))
        {
    ...
    ISPInputData* pInputData)
{
    ...
    for (count = 0; count < m_numIPEIQModulesEnabled; count++)
    {
        ...
        if (CamxResultSuccess != result)
        {
            CAMX_LOG_ERROR(CamxLogGroupPProc, "%s: Failed to Run IQ Config, count %d", NodeIdentifierString(), count);
            break;
        }
    ...
    ISPInputData* pInputData)
{
    ...
    if (TRUE == IsLoopBackPortEnabled())
    {
        ...
        if (NULL != pSelectedAlignmentReferenceParams)
        {
            ...
            CAMX_LOG_VERBOSE(CamxLogGroupPProc,
                             "IPE:%d Perspective ReferenceParams %d , w %d, h %d, c %d, r %d, frameNum %llu",
                             InstanceID(),
    ...
    UINT portId)
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupPProc, "%s num of passes %d", NodeIdentifierString(), m_numPasses);
    ...
    ISPInputData*               pInputData)
{
    ...
    if ((NULL != pImageAlignmentReferenceParams) && (NULL != pGyroAlignmentReferenceParams))
    {
        ...
        if ((NULL != pTuningManager) && (TRUE == pTuningManager->IsValidChromatix()))
        {
            ...
            if (NULL != pChromatix)
            {
                ...
                video_1_0_0::video10_rgn_dataType::fusionStruct* pCoreFusion = &pCoreData->mod_video10_lens_posn_data->
                    lens_posn_data.mod_video10_lens_zoom_data->
                    lens_zoom_data.mod_video10_hdr_aec_data->
                    hdr_aec_data.mod_video10_pre_scale_ratio_data->
                    pre_scale_ratio_data.mod_video10_aec_data->
        ...
        else if (NCLIB_NO_ALIGNMENT == m_currentAlignmentType)
        {
            pSelectedAlignmentReferenceParams = pImageAlignmentReferenceParams;
            pSelectedAlignmentReferenceParams->byPassAlignmentMatrixAdjustement = TRUE;
        }
    }
    else if (NULL != pImageAlignmentReferenceParams)
    {
        pSelectedAlignmentReferenceParams = pImageAlignmentReferenceParams;
    }
    else if (NULL != pGyroAlignmentReferenceParams)
    {
        pSelectedAlignmentReferenceParams = pGyroAlignmentReferenceParams;
    }
...
const SensorMode* IPENode::GetSensorModeData()
{
    ...
    if (FALSE == sensorConnected)
    {
        ...
        if (NULL != pData[2])
        {
            ...
            CAMX_LOG_INFO(CamxLogGroupPProc, "IPE using vendor tag com.qti.sensorbps mode index %d", modeIndex);
        }
        else
        {
            CAMX_LOG_INFO(CamxLogGroupPProc, "Sensor mode index vendor tag not provided! Using default 0!");
        }
    ...
    else
    {
        ...
        CAMX_LOG_INFO(CamxLogGroupPProc, "IPE using pipeline mode index");
    }
