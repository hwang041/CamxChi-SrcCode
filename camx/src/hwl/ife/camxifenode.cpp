...
CamxResult IFENode::UpdateInitSettings()
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        m_mode = DualIFEUtils::EvaluateDualIFEMode(&m_IFEInitData, m_enableBusRead);
    ...
    UINT32  sensorBitWidth)
{
    ...
    *pClockRate = 0;
    ...
    switch(titanChipVersion)
    {
        ...
        default:
            result = CamxResultEUnsupported;
            CAMX_LOG_VERBOSE(CamxLogGroupPower, "Dynamic CSID clock rate calculation not yet added for titan chip version %d",
                titanChipVersion);
            break;
    }
...
CamxResult IFENode::InitialCAMIFConfig()
{
    ...
    for ( UINT32 i = 0; i < m_IFEAcquiredHWInfo.valid_acquired_hw; i++)
    {
        ...
        if (m_IFEAcquiredHWInfo.acquired_hw_id[i] & (IFE0HWMask | IFE1HWMask))
        {
    ...
    if ( TRUE == IFEHWUsed[IFEHWTypeNormal] && TRUE == IFEHWUsed[IFEHWTypeLite] )
    {
        ...
        CAMX_LOG_ERROR(CamxLogGroupISP, "Enabling simultaneous IFE and IFE lite not supported")
    }
    ...
    BOOL          isFS2Enable)
{
    ...
    {
        if (TRUE == pISPInputdata->csidBinningInfo.isBinningEnabled)
        {
                            ...
                            if (static_cast<UINT>(IFESplitID::Left) == IFEIndex) // KMD only uses IFE0 blob for RDI votes
                            {
                                ...
                                if (RDIMaxNum > idx)
                                {
                                    ...
                                    if (0.0f < bytesPerPix)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if (PortSrcTypePDAF == GetOutputPortSourceType(outputPortIndex))
                                            {
                                                ...
                                                if (TRUE == FindSensorStreamConfigIndex(StreamType::PDAF, &PDAFstreamIndex))
                                                {
                                                    ...
                                                    if (PDLibSensorType2 == GetPDAFSensorType())
                                                    {
                                                        pdafRatio = IFEPDAFT2BandwidthRatio;
                                                    }
                                                    else if (PDLibSensorType3 == GetPDAFSensorType())
                                                    {
                                                        pdafRatio = IFEPDAFT3BandwidthRatio;
                                                    }
                                                }
                                            }
                                        }
                                        ...
                                        // Using mnocIBW variable temporarily to calculate
                                        // AB bandwidth values that to be computed for SM7150
                                        pBWPathPerVote->mnocIBBW += static_cast<UINT64>(bandwidth * pdafRatio);
                                        CAMX_LOG_VERBOSE(CamxLogGroupPower,
                                            "ReqId=%llu IFEOutputPortRDIx[%d] bytesPerPix=%f Dims:[%ux%u] "
                                            "externalBWbytesAB=%llu",
    ...
    UINT32 gridType)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupISP, "Invalid ica grid ptr");
    }
}
