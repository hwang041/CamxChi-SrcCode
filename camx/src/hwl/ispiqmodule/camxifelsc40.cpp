    ...
    ISPInputData* pInputData)
{
        ...
        (NULL != pInputData->pHALTagsData))
    {
        ...
        width  = pInputData->sensorData.CAMIFCrop.lastPixel -
            pInputData->sensorData.CAMIFCrop.firstPixel + 1;
        height = pInputData->sensorData.CAMIFCrop.lastLine -
             ...
             (TRUE  == pInputData->forceTriggerUpdate)))
        {
            ...
            {
                m_pState->dependence40Data.fullResWidth  = static_cast<UINT32>(
                    pInputData->sensorData.fullResolutionWidth / pInputData->sensorData.sensorBinningFactor);
                m_pState->dependence40Data.fullResHeight = static_cast<UINT32>(
            ...
            m_pState->dependence40Data.imageWidth     = width;
            m_pState->dependence40Data.imageHeight    = height;
            m_pState->dependence40Data.scalingFactor  = static_cast<UINT32>(pInputData->sensorData.sensorScalingFactor);
            m_pState->dependence40Data.offsetX        = pInputData->sensorData.sensorOut.offsetX;
            m_pState->dependence40Data.offsetY        = pInputData->sensorData.sensorOut.offsetY;
    ...
    if (NULL != pInputData->pHALTagsData)
    {
        ...
        {
            if (((m_AELock != ControlAELockOn) ||
                (m_AELock != pInputData->pHALTagsData->controlAECLock)) &&
                (NULL != pInputData->pStripeConfig->statsDataForISP.pParsedAWBBGStats))
            {
        ...
        m_AELock = static_cast<ControlAELockValues>(pInputData->pHALTagsData->controlAECLock);
        ...
        if (pInputData->pHALTagsData->colorCorrectionMode == ColorCorrectionModeTransformMatrix &&
