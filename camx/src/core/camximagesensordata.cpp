    ...
    UINT          resolutionIndex)
{
    ...
    if (NULL != pFrameDimension)
    {
        ...
        UINT digitalCropleftSize  = (((analogCroppedWidth / horizontalBinning) - (digitalCroppedWidth)) / (EqualCropfactor)) *
                                    (horizontalBinning);
        UINT digitalCroptopSize   = (((analogCroppedHeight / verticalBinning) - (digitalCroppedHeight)) / (EqualCropfactor)) *
                                    (verticalBinning);
    ...
    BOOL                isComboMode)
{
    ...
    for (UINT i = 0; i < pResolutionInfo->resolutionDataCount; i++)
    {
        ...
        pSensorModeData->allModes[i].mipiFlags               =  0;
        for (UINT index = 0; index < pResolutionInfo->resolutionData[i].mipiFlagsCount; index++)
        {
            pSensorModeData->allModes[i].mipiFlags           |=
                                    (1 << static_cast<UINT32>(pResolutionInfo->resolutionData[i].mipiFlags[index]));
            CAMX_LOG_CONFIG(CamxLogGroupSensor, "%d mipiFlags = %d", index, pSensorModeData->allModes[i].mipiFlags);
        }
