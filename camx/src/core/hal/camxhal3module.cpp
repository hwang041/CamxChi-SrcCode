...
HAL3Module::HAL3Module()
{
    ...
    {
        pD = OsUtils::StrStr(&moduleFileName[0], "moorea");
        ...
        if (NumCHIOverrideModules >= fileCount)
        {
            ...
            {
                CAMX_LOG_INFO(CamxLogGroupHAL, "opening CHI Module - %s", &moduleFileName[fileIndexMoorea]);
                m_hChiOverrideModuleHandle = OsUtils::LibMap(&moduleFileName[fileIndexMoorea]);
            }
            else
            {
    ...
    m_pThermalManager = CAMX_NEW ThermalManager();
