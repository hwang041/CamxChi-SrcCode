    ...
    if (NULL == GetInstance(cameraId)->m_pMetadataTemplates[requestTemplateIndex])
    {
            ...
            (requestTemplate <= RequestTemplateManual))
        {
            ...
            if ((NULL != pMetadata) && (NULL != cameraInfo.pPlatformCaps) && (NULL != cameraInfo.pSensorCaps))
            {
                ...
                // Update disable FPS limits
                UINT32 disableFPSLimitsTag;
                BYTE   defaultDisableFPSlimitsValue = 0;

                result = VendorTagManager::QueryVendorTagLocation("org.quic.camera.manualExposure", "disableFPSLimits",
                                                                  &disableFPSLimitsTag);
                CAMX_ASSERT_MESSAGE(CamxResultSuccess == result, "Failed to get vendor tag location for disableFPSLimits");
                if (CamxResultSuccess == result)
                {
    ...
    const HwCameraInfo*          pCameraInfo)
{
    ...
    pDefaultRequest->AERegions.xMin             = pCameraInfo->pSensorCaps->activeArraySize.xMin;
    pDefaultRequest->AERegions.yMin             = pCameraInfo->pSensorCaps->activeArraySize.yMin;
    pDefaultRequest->AERegions.xMax             = pCameraInfo->pSensorCaps->activeArraySize.xMin +
                                                    pCameraInfo->pSensorCaps->activeArraySize.width;
    pDefaultRequest->AERegions.yMax             = pCameraInfo->pSensorCaps->activeArraySize.yMin +
                                                    pCameraInfo->pSensorCaps->activeArraySize.height;
    ...
    pDefaultRequest->AFRegions.xMin   = pCameraInfo->pSensorCaps->activeArraySize.xMin;
    pDefaultRequest->AFRegions.yMin   = pCameraInfo->pSensorCaps->activeArraySize.yMin;
    pDefaultRequest->AFRegions.xMax   = pCameraInfo->pSensorCaps->activeArraySize.xMin +
        pCameraInfo->pSensorCaps->activeArraySize.width;
    pDefaultRequest->AFRegions.yMax   = pCameraInfo->pSensorCaps->activeArraySize.yMin +
        pCameraInfo->pSensorCaps->activeArraySize.height;
    ...
    pDefaultRequest->AWBRegions.xMin            = pCameraInfo->pSensorCaps->activeArraySize.xMin;
    pDefaultRequest->AWBRegions.yMin            = pCameraInfo->pSensorCaps->activeArraySize.yMin;
    pDefaultRequest->AWBRegions.xMax            = pCameraInfo->pSensorCaps->activeArraySize.xMin +
                                                    pCameraInfo->pSensorCaps->activeArraySize.width;
    pDefaultRequest->AWBRegions.yMax            = pCameraInfo->pSensorCaps->activeArraySize.yMin +
                                                    pCameraInfo->pSensorCaps->activeArraySize.height;
