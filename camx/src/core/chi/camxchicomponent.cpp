    ...
    UINT16                 index)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupChi, "Failed to retrieve ChiHistAlgorithmEntry function address. from lib:%s", pLibFileName);
    }
    ...
    UINT16                 index)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupChi, "Failed to retrieve ChiAECAlgorithmEntry function address.")
    }
    ...
    UINT16                 index)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupChi, "Failed to retrieve ChiAFAlgorithmEntry function address.")
    }
    ...
    UINT16                 index)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupChi, "Failed to retrieve ChiAFDAlgorithmEntry function address.")
    }
    ...
    UINT16                 index)
{
    ...
    {
        CAMX_LOG_ERROR(CamxLogGroupChi, "Failed to retrieve ChiASDAlgorithmEntry function address.")
    }
    ...
    UINT*                  pNumExternalComponent)
{
    ...
    if (CSLCameraTitanSocSM7450 != CSLPlatform.socId)
    {
    ...
    else if (CSLCameraTitanSocSM7450 == CSLPlatform.socId)
    {
#if defined(_LP64)
    ...
    if (0 < fileCountTypeOverride)
    {
        ...
        INT    fileIndex        = 0;
        pD                      = OsUtils::StrStr(&chiOverrideSoFileName[0], "moorea");
        PFNCHIQUERYVENDORTAG pQueryVendorTag;

        // pD is NULL if Moorea is not present in first file index
        if (pD != NULL)
        {
            fileIndexMoorea = 0;
            fileIndex = FILENAME_MAX;
        }
        if (CSLPlatform.socId == CSLCameraTitanSocSM7150 || CSLPlatform.socId == CSLCameraTitanSocSM7150P)
        {
            pQueryVendorTag = reinterpret_cast<PFNCHIQUERYVENDORTAG>(CamX::OsUtils::LibGetAddr(
                              CamX::OsUtils::LibMap(&chiOverrideSoFileName[fileIndexMoorea]), "chi_hal_query_vendertag"));
        }
        else
        {
            pQueryVendorTag = reinterpret_cast<PFNCHIQUERYVENDORTAG>(CamX::OsUtils::LibGetAddr(
