...
{
    CamxResult result            = CamxResultSuccess;
    UINT32     grallocBufferSize = 0;

    if (mQtiMapperExtensions != NULL)
    {
        ExtensionError error   = ExtensionError::NONE;
        ...
        {
            CAMX_LOG_WARN(CamxLogGroupFormat, "getSize returned error %d, ignore bufSize validation",
                          error);
        }
        else if (grallocBufferSize < bufSize)
        {
            CAMX_LOG_ERROR(CamxLogGroupFormat, "grallocBufferSize %d is less than TotalSize %d",
                           grallocBufferSize, bufSize);
            result = CamxResultEResource;
        }
        else if (grallocBufferSize > bufSize)
        {
            CAMX_LOG_VERBOSE(CamxLogGroupFormat, "grallocBufferSize %d is greater than TotalSize %d ",
                             grallocBufferSize, bufSize);
        }
    }
