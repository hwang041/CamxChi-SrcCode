...
static const CHAR MultiCameraInfoSectionName[]      = "com.qti.chi.metadataOwnerInfo";
    ...
    CHINODEPROCESSREQUESTINFO* pProcessRequestInfo)
{
    ...
    if (1 < m_numOfLinkedCameras)
    {
        ...
        VOID* pData = ChiNodeUtils::GetMetaData(pProcessRequestInfo->frameNum,
                                                g_vendorTagId.multiCameraIdTagId | InputMetadataSectionMask,
                                                ChiMetadataDynamic,
                                                &g_ChiNodeInterface,
                                                m_hChiSession);
        if (NULL != pData)
        {
            currentCameraId         = *static_cast<UINT32*>(pData);
    ...
    is_output_type* pAlgoResult)
{
    ...
    else
    {
        ...
        gyroAlignmentTrans.perspectiveTransformEnable       = TRUE;
        gyroAlignmentTrans.perspectiveConfidence            = 1;
...
{
    CHIRectangle  sensorCropOnActiveArray;
    WindowRegionF sensorAppliedCropWindow;
    CHIRECT       sensorCropInfo         = m_perSensorData[sensorIdx].cameraConfig.sensorModeInfo.cropInfo;
    CHIRECT       sensorFrameDimension   = m_perSensorData[sensorIdx].cameraConfig.sensorModeInfo.frameDimension;
    UINT32        horizontalBinning      = m_perSensorData[sensorIdx].cameraConfig.sensorModeInfo.horizontalBinning;
    UINT32        verticalBinning        = m_perSensorData[sensorIdx].cameraConfig.sensorModeInfo.verticalBinning;

    // Sensor crop info is in coordinate form, convering coordinte to width and height
    sensorCropOnActiveArray.width        = (sensorCropInfo.width  - sensorCropInfo.left + 1) * horizontalBinning;
    sensorCropOnActiveArray.height       = (sensorCropInfo.height - sensorCropInfo.top  + 1) * verticalBinning;

    // Crop left should consider CSID crop + sensor analog crop + sensor digital crop
    sensorCropOnActiveArray.left         = sensorCropInfo.left * horizontalBinning;
    sensorCropOnActiveArray.left         += sensorFrameDimension.left;
    sensorCropOnActiveArray.left         += (m_perSensorData[sensorIdx].activeArraySize.width -
                                            ((2 * sensorCropOnActiveArray.left) + sensorCropOnActiveArray.width)) / 2;

    // Crop top should consider CSID crop + sensor analog crop + sensor digital crop
    sensorCropOnActiveArray.top          = sensorCropInfo.top * verticalBinning;
    sensorCropOnActiveArray.top          += sensorFrameDimension.top;
    sensorCropOnActiveArray.top          += (m_perSensorData[sensorIdx].activeArraySize.height -
                                            ((2 * sensorCropOnActiveArray.top) + sensorCropOnActiveArray.height)) / 2;
    ...
    sensorAppliedCropWindow.windowLeft   = sensorCropOnActiveArray.left;
    sensorAppliedCropWindow.windowTop    = sensorCropOnActiveArray.top;
    sensorAppliedCropWindow.windowWidth  = sensorCropOnActiveArray.width;
    sensorAppliedCropWindow.windowHeight = sensorCropOnActiveArray.height;
