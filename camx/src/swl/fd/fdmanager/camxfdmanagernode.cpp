    ...
    UINT32             portId)
{
    ...
    if (CamxResultSuccess == result)
    {
        ...
        if (CamxResultSuccess == result)
        {
            ...
            if ((FDManagerInputPortImage == portId) && (0 != previewDimension.width) && (0 != previewDimension.height))
            {
                if ((previewDimension.width  <= m_baseFDHWDimension.width) &&
                    (previewDimension.height <= m_baseFDHWDimension.height))
                {
                    pInputBufferRequirement->minWidth  = previewDimension.width;
                    pInputBufferRequirement->minHeight = previewDimension.height;
                ...
                else
                {
                    ...
                    pInputBufferRequirement->maxWidth  = previewDimension.width;
                    pInputBufferRequirement->maxHeight = previewDimension.height;
                }

                pInputBufferRequirement->planeAlignment[0].strideAlignment   = 1;
                pInputBufferRequirement->planeAlignment[1].strideAlignment   = 1;
                pInputBufferRequirement->planeAlignment[0].scanlineAlignment = 1;
                pInputBufferRequirement->planeAlignment[1].scanlineAlignment = 1;
            }
            else
            {
                pInputBufferRequirement->minWidth  = width;
    ...
    FDPerFrameSettings*     pPerFrameSettings)
{
    ...
    if (((TRUE  == m_bEnableSwDetection) && (NULL == m_hFDEngineSWHandle)) ||
        ((FALSE == m_bEnableSwDetection) && (NULL == m_hFDEnginePriHandle)))
    {
        ...
        else
        {
            ...
            else if (FDFPFiltering == pPerFrameSettings->FDManagerExecutionSequence)
            {
                ...
                PrintFDResults("FDEngineResults-Unstabilized", pPerFrameSettings->requestId, m_FDFrameWidth, m_FDFrameHeight,
                               unstabilizedROIResults.numFacesDetected, &(unstabilizedROIResults.faceInfo[0]));

                // Sort faces from largest to smallest. The design is to run facial attribute detection on the largest faces,
                ...
                PrintFDResults("AfterSorting-Unstabilized", pPerFrameSettings->requestId, m_FDFrameWidth, m_FDFrameHeight,
                               unstabilizedROIResults.numFacesDetected, &(unstabilizedROIResults.faceInfo[0]));
    ...
    FDMetaDataFaceContourResults*  pFaceContourResults)
{
    ...
    UINT   EISTag[] =
    {
        tagEISEnabled | UsecaseMetadataSectionMask
    };
    const UINT length           = CAMX_ARRAY_SIZE(EISTag);
    VOID*      pEISData[length] = { 0 };
    UINT64     offset[length]   = { 0 };

    if (CamxResultSuccess == result)
    {
        result = GetDataList(EISTag, pEISData, offset, length);
        if (CamxResultSuccess != result)
        {
            CAMX_LOG_ERROR(CamxLogGroupCore, "Failed to get EIS enable setting (%d)", result);
        }
        else
        {
            if (NULL != pEISData[0])
            {
                EISEnabled = *reinterpret_cast<BOOL*>(pEISData[0]);
    ...
    if ((CamxResultSuccess == result) && (TRUE == EISEnabled) &&
        ((0.0 > m_EISMarginRatio.widthMargin) || (0.0 > m_EISMarginRatio.heightMargin)))
    {
    ...
    BOOL*                                  pProcessHandled)
{
    ...
    else if ((TRUE == GetStaticSettings()->enableFDManagerThreading) && (FALSE == m_bEnableSwDetection))
    {
