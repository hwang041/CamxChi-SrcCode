    ...
    BOOL                   hasExplicitDependencies)
{
    ...
    if (pDependency->propertyDependency.count > 0)
    {
        pDependency->dependencyFlags.hasPropertyDependency = TRUE;
    ...
    ExecuteProcessRequestData* pExecuteProcessRequestData)
{
    ...
    BOOL                    bisFSCaptureRequest = FALSE;
    const StaticSettings*   pStaticSettings     = GetStaticSettings();
        ...
        (NULL != pExecuteProcessRequestData->pNodeProcessRequestData->pCaptureRequest))
    {
        ...
        bisFSCaptureRequest = IsFSCaptureRequest();
        if (TRUE == bShouldSetDependency)
        {
            BOOL hasExplicitDependencies = TRUE;
            if (TRUE == bisFSCaptureRequest)
            {
        ...
        {
            if ((FALSE == IsStatsNodeEnabled()) || (TRUE == bisFSCaptureRequest))
            {
                if (NULL != m_pActuator)
                {
                ...
                else if (TRUE == bisFSCaptureRequest)
                {
                ...
                else
                {
                    ...
                    UINT InputAECData[] =
                    {
                        ...
                        disableFPSLimitsTag | InputMetadataSectionMask  // 3
                    };
                    ...
                    if ((0 != inputFrameDuration) && (FALSE == bisFSCaptureRequest))
                    {
                        // Seamless in-sensor won't be triggered in this mode, so pass FALSE into
                        // function ExposureToLineCount and GetFrameLengthLines in the following.
                        updateParam.currentFrameLengthLines = GetSensorDataObject()->ExposureToLineCount(inputFrameDuration,
                                                                                                    m_currentResolutionIndex,
                                                                                                    currentSeamlessType);
                        BOOL disableFPSLimits = (NULL != pInputData[3] ? *(static_cast<BYTE*>(pInputData[3])) : FALSE);
                        if (FALSE == disableFPSLimits)
                        {
