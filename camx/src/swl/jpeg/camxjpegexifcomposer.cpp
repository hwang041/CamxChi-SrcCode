    ...
    if (neededSize < static_cast<UINT32>(m_pOutputBuf->GetPlaneSize(0))) {
        ...
        if (NULL != m_pStatsDebugDataAddr) {
            CAMX_LOG_CONFIG(CamxLogGroupDebugData,
                            "DebugDataAll: EXIF writing total: %p, size: %u, 3A enable: %u, Tuning enable: %u "
        ...
        {
            CAMX_LOG_INFO(CamxLogGroupDebugData, "DebugDataAll: Not writing 3A debug data or Tuning metadata. 3A enable: %u "
                          "Tuning enable: %u",
                          pStaticSettings->enable3ADebugData, pStaticSettings->enableTuningMetadata);
        }
    }
    else
    {
