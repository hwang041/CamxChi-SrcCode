    ...
    AECCommandOutputParam*  pOutput)
{
    ...
    CAMX_LOG_VERBOSE(CamxLogGroupAEC, "AEC: Command = %s EngineState = %s, PreflashState = %s",
                     CamxAECEngineCommandStrings[static_cast<UINT8>(command)],
    ...
    switch (command)
    {
        ...
        case AECCommand::SetFPSRange:
            ...
            break;
        case AECCommand::SetAELock:
            lockSetting = static_cast<AECAlgoLockType>(pInput->pHALParam->AELock);
            result = SetSingleParamToAlgorithm(AECAlgoSetParamAECLock, &lockSetting, sizeof(AECAlgoLockType));
            break;
        default:
