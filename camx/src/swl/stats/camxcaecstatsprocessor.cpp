    ...
    const StatsProcessRequestData* pStatsProcessRequestDataInfo)
{
    ...
    else
    {
        ...
        if (CamxResultSuccess == result)
        {
            // Set Lock first before algo queries for start up exposure
            inputParam.pHALParam = &m_HALParam;
            result = m_pAECEngine->HandleCommand(AECCommand::SetAELock, &inputParam, &m_outputParam);
        }
    ...
    ) const
{
    ...
    if (NULL != m_pStatsAECPropertyReadWrite->GetPropertyData(AECReadTypeInputControlAETargetFpsRange))
    {
    ...
    ) const
{
    ...
    if (0 != pTouchROIInfo->weight)
    {
        ...
        if (touchROI.x >= statsROI.left)
        {
            touchROI.x -= statsROI.left;
        }
        if (touchROI.y >= statsROI.top)
        {
